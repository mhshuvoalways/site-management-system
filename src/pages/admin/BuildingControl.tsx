...
export function BuildingControlPage() {
  ...
  const handleCameraCapture = async (e: React.ChangeEvent<HTMLInputElement>, isEdit = false) => {
    ...
    let locationAddress = "";
    if (geo) {
      locationAddress = await reverseGeocode(geo.lat, geo.lng);
    }
  ...
  const uploadPhotosForReport = async (reportId: string, uploads: PhotoUpload[]) => {
    for (const upload of uploads) {
      ...
      await supabase.from("building_control_photos").insert({
        building_control_id: reportId,
        photo_url: publicUrlData.publicUrl,
        notes: upload.notes,
        created_by: profile!.id,
        latitude: upload.latitude ?? null,
        longitude: upload.longitude ?? null,
        location_address: upload.locationAddress ?? null,
        taken_at: upload.takenAt ?? null,
      });
  ...
  const handleConfirmDeletePhoto = async () => {
    ...
    const fileName = deletePhotoDialog.photoUrl.split("/building-control-photos/")[1];
    if (fileName) {
  ...
  const handleUpdateReport = async (e: React.FormEvent) => {
    ...
    if (editPhotoUploads.length > 0) {
      await uploadPhotosForReport(editingReport.id, editPhotoUploads);
    }
  ...
  const generateReportPdf = async (reportsToDownload: BuildingControl[]) => {
    ...
    for (let ri = 0; ri < reportsToDownload.length; ri++) {
      ...
      doc.text("Site Photos Report", margin, y);
      y += 10;
      ...
      if (report.photos && report.photos.length > 0) {
        ...
        for (const photo of report.photos) {
          ...
          if (photo.location_address || photo.taken_at) {
            ...
            doc.setTextColor(100);
            const locationLine = [
              photo.location_address ? `📍 ${photo.location_address}` : "",
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      {formatLastModified(report) && (
                        <p className="text-xs text-amber-600 mt-1">{formatLastModified(report)}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="relative">
                      ...
                      {showDownloadMenu === report.id && (
                        <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-10 w-40">
                          ...
                          <button
                            onClick={() => handleDownloadReport(report, "images")}
                        ...
                        <div key={photo.id} className="bg-gray-50 rounded-lg overflow-hidden border border-gray-200 group">
                          <div className="relative">
                            ...
                            <div className="absolute top-2 right-2">
                              <button
                                onClick={() => openDeletePhotoDialog(photo.id, photo.photo_url)}
            ...
            <form onSubmit={handleCreateReport} className="space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#0db2ad] transition">
                    <input
                      ...
                      accept="image/*"
                      capture="environment"
                      onChange={(e) => handleCameraCapture(e, false)}
                      className="hidden"
                      id="camera-capture"
                    />
                    ...
                    {photoUploads.map((upload, index) => (
                      <div key={index} className="bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
                        <div className="relative">
                          ...
                          {upload.locationAddress && (
                            <div className="absolute bottom-2 left-2 bg-black bg-opacity-60 text-white text-xs px-2 py-1 rounded flex items-center space-x-1 max-w-[80%]">
                              <MapPin className="w-3 h-3 flex-shrink-0" />
                              <span className="truncate">{upload.locationAddress}</span>
                            </div>
                          )}
                        </div>
                        <div className="p-3">
              ...
              <div className="flex space-x-3 pt-4">
                <button
                  ...
                  onClick={() => {
                    setShowNewModal(false);
                    photoUploads.forEach((upload) => URL.revokeObjectURL(upload.preview));
                    setPhotoUploads([]);
                  }}
                    ...
                    {editingReport.photos.map((photo) => (
                      <div key={photo.id} className="bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
                        <div className="relative">
                          ...
                          <button
                            type="button"
                            onClick={() => handleDeleteExistingPhoto(photo.id, photo.photo_url)}
                            className="absolute top-2 right-2 p-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition"
                          >
                            <X className="w-4 h-4" />
                          </button>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#0db2ad] transition">
                    <input
                      ...
                      accept="image/*"
                      capture="environment"
                      onChange={(e) => handleCameraCapture(e, true)}
                      className="hidden"
                    ...
                    {editPhotoUploads.map((upload, index) => (
                      <div key={index} className="bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
                        <div className="relative">
                          ...
                          <button
                            type="button"
                            onClick={() => removeEditPhotoUpload(index)}
                            className="absolute top-2 right-2 p-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition"
                          >
                            <X className="w-4 h-4" />
                          </button>
            ...
            <div className="mt-4 space-y-2 text-center">
              ...
              {viewingPhoto.taken_at && (
                <p className="text-white text-sm opacity-60">
                  Taken: {new Date(viewingPhoto.taken_at).toLocaleString()}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="relative">
            ...
            {showBulkDownloadMenu && (
              <div className="absolute bottom-full mb-2 left-0 bg-white text-gray-900 border border-gray-200 rounded-lg shadow-lg z-10 w-44">
                <button
                  onClick={() => handleBulkDownloadReports("pdf")}
                  className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 rounded-t-lg"
                >
                  Download as PDF
                </button>
                <button
                  onClick={() => handleBulkDownloadReports("images")}
                  className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 rounded-b-lg"
                >
                  Download as Images
                </button>
